    ...
    function attachEditorToolbarListeners(container) {
      ...
      quoteBtns.forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          span.textContent = " — Author Name"; // Default author placeholder

          // Handle existing text selection
          if (!range.collapsed) {
            const quoteText = range.toString();
            paragraph.textContent = quoteText;
            paragraph.appendChild(span); // Append span inside <p> before it closes
            blockquote.appendChild(paragraph);
            range.deleteContents();
          } else {
            // Placeholder blockquote if no text is selected
          ...
          // Move caret outside the blockquote for further editing
